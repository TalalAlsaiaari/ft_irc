...
int main()
{
	...
	// printf("test family: %d\n", test->sin_family);
	// printf("test port: %d\n", ntohs(test->sin_port));
	// printf("test IP address: %s\n", inet_ntoa(test->sin_addr));
	// host = gethostbyaddr(inet_ntoa(test->sin_addr), test->sin_len, test->sin_family);
	// printf("client host name: %s\n", host->h_name);
	// int i = 0;
	// while (host->h_addr_list[i])
	// 	printf("client host aliases: %s\n", host->h_aliases[i++]);
	// test2 = (struct sockaddr *)&their_addr;
	// printf("test2 family: %d\n", test2->sa_family);
	// printf("test2 data: %c\n", test2->sa_data[0]);
	len = strlen(serv_msg);
